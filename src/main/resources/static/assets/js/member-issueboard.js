...
function dragDrop() {
    ...
    if(issueStatus !== "TODO" && issueStatus !== "COMPLETED"){
        if(status=="COMPLETED"){
            if (existingTasksWrapper) {
                ...
                let currentAssignId=currentText.innerHTML;
                updateAssignIssueStatus(currentAssignId, status).then(()=>{
        ...
        if(status=="INPROGRESS"){
            ...
            console.log("TODO Dropping is unable!");
        }
    }else {
...
// Usage:
async function updateAssignIssueStatus(assignIssueId, newStatus) {
    try {
        // Assuming you have an endpoint on the server to handle the update
        const response = await fetch(`/assign-issue/${assignIssueId}`, {
            method: 'PUT',
            headers: {
        ...
        if (response.ok) {
            console.log(`AssignIssue ${assignIssueId} status updated to ${newStatus}`);
        } else {
            console.error(`Failed to update AssignIssue ${assignIssueId} status`);
        }
    } catch (error) {
